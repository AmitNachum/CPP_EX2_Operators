...
squaredMatrix Squared_Matrix::squaredMatrix::operator*(const squaredMatrix &other)const{
...
{
    squaredMatrix operator*(double scalar, squaredMatrix &other){
    squaredMatrix res = squaredMatrix(other.n);
    ...
    for (size_t i = 0; i < other.n; i++)
    {
        ...
        {
            res.matrix[i][j] = 0;
            double val = other.matrix[i][j] * scalar;
            res.matrix[i][j] = val;
        }
...
squaredMatrix squaredMatrix::operator*(double scalar) const {
    
    squaredMatrix result(n);
    for (size_t i = 0; i < n; i++)
        for (size_t j = 0; j < n; j++)
...
squaredMatrix squaredMatrix::operator~() const {
    squaredMatrix result(n);
    for (size_t i = 0; i < n; i++)
        for (size_t j = 0; j < n; j++)
